...
OUTPUT_CSV = "D:\\自然语言处理\\期末大作业\\promptattack_W1_victimSVM.csv"
SLEEP_TIME = 1.5
...
DIALOGUE_PERTURBATIONS = {
    ...
    "C2": "改变对话中1-2个字母，让文本看起来更自然。",
    "C3": "在对话末尾添加1个表情符号（如:)、^_^），不改变语义。",
    
    "W1": "替换对话中7-9个关键词为同义词或更正式的表达方式。",
    "W2": "删除对话中1-2个不重要的、冗余的词。",
    "W3": "添加1-2个让对话更友好、更自然的过渡词。",
...
def fidelity_filter(original: str, adversarial: str, scorer, 
                   ...
                   max_fidelity: float = 0.97) -> Tuple[bool, Dict]:
...
任务描述：{TASK_DESCRIPTION}"""
    ...
    # 修改攻击目标部分，移除让LLM做出错误分类的要求
    ao_section = f"""# 攻击目标 (Attack Objective)
你的任务是生成一个修改后的对话，满足以下条件：
1. 保持原对话的语义不变；
2. 修改后的对话应该看起来自然，不引起怀疑；
3. 原始标签是"{true_label_chinese}"，你需要针对SVM分类器生成对抗样本。"""
...
def generate_adversarial_with_attack_loop(
    ...
    for attempt in range(max_attempts):
        print(f"    生成尝试 {attempt + 1}/{max_attempts}")
        ...
        # 保真度检查（BERTScore）
        passed, fidelity_result = fidelity_filter(dialogue, adv_dialogue, scorer, 0.80, 0.97)
        if not passed:
        ...
        print(f"    ✓ 生成成功！保真度: {fidelity_result['fidelity']:.4f}")
        return adv_dialogue, perturbation_id, fidelity_result, True
    
    # 返回最好的生成结果
    if best_gen_success:
        print(f"    ⚠️ 生成成功，返回最佳对抗样本")
        return best_adv_dialogue, best_perturbation, best_fidelity_result, True
    else:
        print(f"    ❌ 所有尝试均失败")
        return dialogue, "none", {'fidelity': 1.0, 'method': 'failed'}, False
...
def create_complete_failure_result(dialogue, true_label, parts_count):
    """创建完全失败结果（生成失败）"""
    true_label_chinese = "诈骗" if true_label == True else "非诈骗"
    return {
        "original_dialogue": dialogue,
...
# =========================
# 13. 主攻击流程（简化版，不包含原始预测检查）
# =========================

def generate_for_dialogue_simple_version(
    dialogue: str,
    true_label: bool,  # 标准化后的布尔值
    ...
    for perturbation_id in selected_perturbations:
        ...
        try:
            adv_dialogue, used_perturbation, fidelity_result, gen_success = generate_adversarial_with_attack_loop(
                dialogue, perturbation_id, scorer, true_label
            )
        except RuntimeError as e:
            # API调用失败，向上抛出异常
        ...
        if gen_success:
            ...
            result = {
                "original_dialogue": dialogue,
                "adversarial_dialogue": adv_dialogue,  # 这里保存对抗样本
    ...
    if best_result is not None:
        ...
        # 全部失败
        print(f"  ❌ 所有扰动类型均失败")
        return create_complete_failure_result(dialogue, true_label, len(parts))


# =========================
# 14. 批量处理主程序（支持断点续传）- 简化版
# =========================

def main():
    """主函数：批量生成对抗样本 - 简化版，不包含预测检查"""
    print("=" * 80)
    print("对抗样本生成器 (诈骗检测任务 - 简化版)")
    print("简化版：不检查原始预测，直接生成对抗样本")
    print("=" * 80)
        ...
        for idx in range(processed_count, len(df)):
            ...
            try:
                ...
                if scorer is not None:
                    result = generate_for_dialogue_simple_version(
            ...
            if len(current_batch_results) >= BATCH_SIZE:
                ...
                # 计算本批统计
                batch_gen_success = sum(1 for r in current_batch_results if r.get('generation_success', False))
                batch_gen_fail = BATCH_SIZE - batch_gen_success
                
                print(f"本批统计:")
                print(f"  生成成功的样本数: {batch_gen_success}")
                print(f"  生成失败的样本数: {batch_gen_fail}")
                print(f"  成功率: {batch_gen_success/BATCH_SIZE*100:.1f}%")
                
                # 计算总体统计
                all_gen_success = sum(1 for r in all_results if r.get('generation_success', False))
                total_processed = len(all_results)
                if total_processed > 0:
                    total_success_rate = all_gen_success / total_processed * 100
                else:
                    total_success_rate = 0
                
                print(f"累计统计:")
                print(f"  总处理样本: {total_processed}")
                print(f"  生成成功的样本数: {all_gen_success}")
                print(f"  总体成功率: {total_success_rate:.1f}%")
                
                # 计算平均BERTScore（生成成功的）
                gen_success_samples = [r for r in all_results if r.get('generation_success', False)]
                if gen_success_samples:
                    avg_f1_gen = sum(r.get('bert_f1', 0) for r in gen_success_samples) / len(gen_success_samples)
                    avg_precision_gen = sum(r.get('bert_precision', 0) for r in gen_success_samples) / len(gen_success_samples)
                    avg_recall_gen = sum(r.get('bert_recall', 0) for r in gen_success_samples) / len(gen_success_samples)
                    
                    print(f"  所有生成成功样本的平均BERT-F1: {avg_f1_gen:.4f}")
                    print(f"  所有生成成功样本的平均BERT-Precision: {avg_precision_gen:.4f}")
                    print(f"  所有生成成功样本的平均BERT-Recall: {avg_recall_gen:.4f}")
    ...
    if all_results:
        try:
            ...
            # 1. 生成成功率统计
            gen_success_df = final_df[final_df['generation_success'] == True]
            total_gen_success = len(gen_success_df)
            total_processed = len(final_df)
            
            if total_processed > 0:
                gen_success_rate = total_gen_success / total_processed * 100
            else:
                gen_success_rate = 0
            
            print(f"【生成成功率】:")
            print(f"  总处理样本: {total_processed}")
            print(f"  生成成功的样本数: {total_gen_success}")
            print(f"  生成成功率: {gen_success_rate:.2f}%")
            ...
            if len(fraud_df) > 0:
                fraud_gen_success = fraud_df[fraud_df['generation_success'] == True]
                fraud_gen_rate = len(fraud_gen_success) / len(fraud_df) * 100 if len(fraud_df) > 0 else 0
                print(f"  诈骗样本: 总数={len(fraud_df)}, 生成成功={len(fraud_gen_success)}, 成功率={fraud_gen_rate:.2f}%")
            
            if len(non_fraud_df) > 0:
                non_fraud_gen_success = non_fraud_df[non_fraud_df['generation_success'] == True]
                non_fraud_gen_rate = len(non_fraud_gen_success) / len(non_fraud_df) * 100 if len(non_fraud_df) > 0 else 0
                print(f"  非诈骗样本: 总数={len(non_fraud_df)}, 生成成功={len(non_fraud_gen_success)}, 成功率={non_fraud_gen_rate:.2f}%")
            ...
            # 显示成功案例
            show_successful_examples_simple_version(gen_success_df)
...
def show_successful_examples_simple_version(results_df: pd.DataFrame, n: int = 5):
    """展示成功的生成示例（简化版）"""
    if len(results_df) == 0:
        print("没有成功的生成示例")
        return
    ...
    print(f"\n{'='*80}")
    print(f"成功生成示例 (前{min(n, len(results_df))}个)")
    print(f"{'='*80}")
